...
merge; `autonomy.mode` decides whether `ship` asks before starting the next one. → `feat/plan.md`, [CONFIGURATION][ctr]
...
**FLOW configuration** — shared `FLOW.md` plus the optional active `FLOW.<harness>.md` overlay:
`tracker`, `git`, `autonomy`, `quality`, `agents`, `models`, `data`, `conventions`, `notes`,
`knowledge`, `observability`. Every command merges them first; effective empty keys auto-detect or
use the stated default. Generated by `/flow:init`; `/flow:doctor` prints each resolved value and its
source, then checks the machine against it. → [CONFIGURATION](CONFIGURATION.md)

**Knowledge sources** — the effective `knowledge` section names a tool per role: `search`
(context for a query, one or several providers in parallel), `stage` (record a finding for the
branch), `read_staging`, `save`. Any MCP, CLI or skill fits — `domain-memory`, `codegraph`, a search
over `docs/adr`. Empty roles degrade silently (no lookup, findings stay in the artifacts,
`KNOWLEDGE.md` instead of a store).
→ [CONFIGURATION §knowledge][cdm]
...
[ctr]: CONFIGURATION.md#multi-prpr-trains
...
[cdm]: CONFIGURATION.md#knowledge
