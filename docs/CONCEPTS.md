...
`models`, `data`, `conventions`, `notes`, `knowledge`, `observability`. Every command reads it
first; empty keys auto-detect or use the stated default. Generated by `/flow:init`; `/flow:doctor` prints it resolved and
checks the machine against it. → [CONFIGURATION](CONFIGURATION.md)

**Knowledge sources** — the `knowledge` section of `FLOW.md` names a tool per role: `search`
