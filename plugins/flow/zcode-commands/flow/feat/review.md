...
- **Gather the material once, not once per reviewer.** `npx flow-workflows@<version> bundle --checks --harness zcode`
...
- **`--harness zcode` is not decoration.** Without it the CLI reads `FLOW.md` alone and never
...
- **`agents.exec_cmd` set → the panel runs from the CLI.** Not a choice to weigh: run
  `npx flow-workflows@<version> review --harness zcode`, which spawns that command once per role over the pack above — **one
