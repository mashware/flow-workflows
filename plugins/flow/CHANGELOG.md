...
**In short**
- **`npx flow-workflows install opencode`** — one command, nothing to clone. Same for `gemini`, `hermes` and `codex`.
- **Updating is that same line again**: it sweeps the previous version's files before copying, so a command dropped upstream stops being invocable instead of lingering for ever.
- **`npx flow-workflows check`** answers the question nobody could answer before — what your harnesses actually run, versus the newest release.
- **Claude Code and Codex are unchanged**: they have their own marketplaces, and `npx flow-workflows install claude` just prints the two marketplace lines rather than pretending npm is the way.
- **Two installers, verified identical**: `bin/cli.mjs` for npx, `adapters/install.sh` for a clone. The preflight installs every harness both ways into throwaway `HOME`s and fails if the trees differ by one byte.

Until now the honest answer to "how does an opencode user update flow?" was: clone the repo, `git
pull`, re-run `adapters/install.sh`, and remember to do it — the word *update* did not appear once
in the adapter documentation. Claude Code has a marketplace and a hook that tells you a new version
landed; Codex has its own. The other three had first-class installation and no updating at all.

opencode has no registry for commands — they are markdown files in `~/.config/opencode/commands/`,
and nothing distributes them. So the distribution cannot come from the harness; it has to come from
here. npm is the one channel all three already have: Node is a given, `npx` needs no install step,
and the package carries the adapters plus the three plugin files the installer reads.

The install now also copies this harness's **config fragment** and **`FLOW.template.md`** into
`~/.claude/flow/`, rather than printing a path into the package. Under `npx` that directory is
deleted the moment the command returns, so every path it used to print would have been dead on
arrival.

Two implementations of one installer is a drift risk, and the repo's answer to drift is mechanical:
`adapter-smoke.py` runs both against throwaway `HOME`s and compares the resulting trees file by
file and hash by hash. `package.json` is generated by `adapter-build.py` from the plugin manifest,
so the published version cannot lag behind the plugin's.
