---
name: flow-core
description: Shared rules every /flow command relies on — resolving FLOW.md plus its harness overlay, model keys, autonomy modes and hard gates, how a stop reads, the live panel and the 00-summary.md handoff, and the report contract every delegated brief carries. Load once per session before running any /flow command.
---
...
**This file belongs to flow `0.80.0`.** Compare it once, at the start of the session, against
