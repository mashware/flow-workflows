...
- More than 2 fix steps → the §2.0bis plan is already written: tick its numbered steps off in `04-fix.md` where they stand, and show them in the live panel's step block (flow-core §4.1 (2b), patched per §4.4 (f)); a harness task list, if there is one, goes on top as `/flow:feat:build §2.1` says. One task per step, same order, marked `in_progress` and `completed` as they go — not batched.
