...
**Sensitive surface** = the globs in `quality.sensitive_paths` from `FLOW.md`; empty → auth/authorization, secrets, payments/billing, personal/sensitive data, a public API/contract shape, or a DB migration/schema change. Which surfaces carry risk is the repo's call, not this command's.
