...
- If `git.worktree_try_cmd` is set in `FLOW.md` and the fix was developed in a worktree (`meta.json.worktree` not null), offer it once so the user can test this branch against the synced environment: "run `<worktree_try_cmd with {BRANCH}=meta.branch>`; `<git.worktree_back_cmd>` to return." Suggest, do not run it yourself. Empty key = skip silently.
