...
- Si `size` es `XS`, sugiere saltar a `/flow:bug:review` salvo que el usuario insista.

## 2. Trabajo

**Test de regresión obligatorio**: lanza el agente de `agents.testing` de FLOW.md (si está vacío, `Agent general-purpose` con el rol de escribir tests) con el encargo:
...
4. Si tocaste seguridad o autenticación: lanza en paralelo el agente de `agents.security` de FLOW.md sobre los archivos del arreglo; si está vacío, usa `Agent general-purpose` con rol de seguridad en el prompt.
...
- Si verde: `phase = "validate"`, añade a `phases_done`. Sugiere `/flow:bug:review`.
