...
- **What a review may spend** (`agents.fanout_max`, `agents.budget_max`): **always write both keys with their value, even when it is the default** (`4` and `12`). They are the only brake on what a review costs, and a key that is absent from `FLOW.md` is a default nobody can see, edit, or lower on the day the bill arrives. Say what each does in one line — `fanout_max` bounds one parallel round, `budget_max` bounds the whole command — and that a repo whose reviews feel expensive lowers `budget_max` first (`8` is a frugal review, `6` is a cheap one; every phase the ceiling skips is named in the artifact). Offer `quality.review_depth: light` alongside for anyone who wants the floor.
