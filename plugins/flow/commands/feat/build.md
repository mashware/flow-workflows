...
**Antes de cargar skills, de crear tasks, de cualquier edición**, redacta un brief en lenguaje **de negocio** (no técnico) específico de **esta MR/PR concreta** (la `in_progress` de `meta.json.mrs`, no de toda la feature):
...
- <scope tentador que queda fuera>.
```
...
**Pregunta al usuario con `AskUserQuestion`** si el brief refleja lo que espera:
...
**Si estás en un build multi-MR/PR**: limítate a lo que toca la MR/PR actual según `04-mr-plan.md`. Cualquier código que pertenezca a una MR/PR posterior es scope creep; recórtalo o aíslalo tras feature flag / código muerto temporal según el plan. Si no se puede aislar, pausa y vuelve a `/feat:plan` para recortar.

Decide modo de ejecución:

- **Single-thread (XS/S/M)**: implementas tú mismo, paso a paso, usando subagentes solo como consultores puntuales si te bloqueas: el agente de `agents.architecture` de `FLOW.md` para dudas de capa (o `Agent general-purpose` si está vacío), y el de `agents.persistence` para dudas de consultas/mappings (o `Agent general-purpose` si está vacío).
- **Delegación parcial (M/L con piezas claras)**: usa `Agent` para endpoints aislados, y el agente de `agents.testing` de `FLOW.md` en paralelo para preparar la suite (o `Agent general-purpose` si está vacío). Pasa el `03-design.md` íntegro en el prompt para que no inventen.

Usa `TaskCreate` para trackear los pasos del `Plan de implementación` del diseño. Marca cada paso como `in_progress` al empezar y `completed` al terminar — no batchees.

### 2.2 Checkpoints (commits locales bajo confirmación del usuario)

**Regla dura**: el agente **no hace `git commit` por su cuenta** durante `/feat:build`. Los commits son **opt-in del usuario** — sin tu confirmación explícita, los cambios se quedan en el working tree para que puedas validarlos antes (probar la UI, ejecutar el flujo, leer el diff).

**Tras completar cada paso del `TaskCreate`**, el agente:

1. **Marca el paso como `completed`** en el `TaskCreate`.
2. **Reporta al usuario** un resumen del paso (≤ 5 líneas):
   ```
   ...
   ```
3. **No commitea**. Espera a que tú digas qué hacer. Posibilidades:
   ...
   - **"Espera, valido"** → el agente se queda quieto. Tú validas a tu ritmo. Cuando vuelvas, decides commit o ajuste.
   - **"Hay que cambiar X"** → el agente ajusta. El commit del paso queda pendiente hasta que vuelvas a dar OK.
   - **"Sigue sin commitear, agrupamos luego"** → el agente arranca el paso siguiente sin commit. Los cambios se acumulan en el working tree (riesgo: si hay corte en caliente §2.3, hay menos puntos limpios).

Reglas para cuando sí se commitea:
- Un commit por paso (cuando se hace). No batchees varios pasos en un commit salvo que tú lo pidas explícitamente.
- `--no-verify` está permitido **solo en commits WIP** (los hooks lentos correrán al final en `/feat:review` y en el commit definitivo de `/feat:ship`).
- Estos commits se aplastan al mergear (si `git.squash` es `true`), así que no tienen que ser bonitos — son solo unidades cortables.
- Si un paso queda a medias (te interrumpen, cambias de enfoque) y pides commit, el mensaje lleva sufijo: `WIP <TICKET>: <paso> (parcial)`.

**Por qué este modelo**: dos motivos. (1) Validas localmente antes de que nada quede commiteado, así no acabas con una rama llena de commits sin haber visto los cambios en ejecución. (2) Si decides commitear de vez en cuando, los commits WIP siguen sirviendo como unidades cortables para §2.3. Si decides no commitear hasta el final, pierdes esa granularidad — es tu decisión, no del agente.

### 2.3 Termómetro de tamaño y corte en caliente

**Tras cada paso completado** (haya commit o no), compara el tamaño real con la estimación de la MR/PR actual en `meta.json.mrs`. Mira commits + staged + unstaged, no solo commits:
...
git status --short | wc -l            # archivos modificados/sin trackear
```
...
Si se supera cualquiera, **pausa** y pregunta al usuario con `AskUserQuestion` (las opciones, en este orden):
...
0. **Si hay cambios sin commitear** en el working tree: avisa al usuario y pídele decidir antes de cortar. O bien commitea lo que está terminado como WIP del paso correspondiente, o bien lo guarda aparte (`git stash`) para que no se mezcle con la siguiente MR/PR. Sin esto, el corte deja cambios sueltos que pertenecen a uno u otro lado sin saber a cuál.
1. Identifica un punto de corte: el último commit WIP donde la pieza es coherente y mergeable (un sub-objetivo cerrado: "endpoint y DTO listos", "migración aplicada", "tests del flujo X verdes"). No cortes a mitad de un cambio.
2. Edita `meta.json.mrs`:
   - La MR/PR actual mantiene `n` y `title`, ajusta `lines_est` y `files_est` a lo real, y queda `in_progress`.
   - Inserta una nueva con `n` siguiente (renumerando las posteriores si hay), `title` describiendo lo restante, `status: "pending"`, y nuevas `lines_est` y `files_est` (orientativas).
3. Edita `04-mr-plan.md`: corta la entrada original en dos, mantiene la justificación de mergeable-solo para ambas mitades.
4. Anota en `05-implementation.md` bajo "Corte en caliente": fecha, motivo, qué se queda y qué pasa a la siguiente.
5. **No reescribas historia con `git rebase`**: los commits WIP que pertenecen a la siguiente MR/PR se quedan en la rama actual. Cuando llegue el momento de construir la siguiente, partiremos desde una rama nueva sobre la base, y esos commits se trasladan con `git cherry-pick` o equivalentes. Esto se documenta y se ejecuta en `/feat:ship` o al arrancar el siguiente `/feat:build`.

**Si ya hubo un corte y vuelves a desbordar**: pregunta al usuario antes de cortar de nuevo — un segundo corte sobre la misma MR/PR es señal de que el plan está mal, no de que la estimación esté un poco baja. La opción correcta probablemente es **3 (reabrir plan)**.

### 2.4 ¿Sale algo fuera del brief?

Si durante el build aparece la tentación de añadir algo que **no está en el brief de §2** ("ya que estoy aquí…", "este test me cubriría también X…", "este renombrado mejoraría Y…"):

**Pausa antes de hacerlo** y pregunta al usuario con `AskUserQuestion`:
...
La regla: lo no previsto **siempre** pasa por el usuario antes de meterse en código. Es lo que filtra el scope creep que es invisible al hacer code review (porque ya está mezclado con el resto).
...
<cosas que surgieron durante el build y se decidió NO incluir; cada una con una línea: "qué" + "por qué tiene sentido como ticket propio">
```
...
**Pausa el build y vuelve a `/feat:design`** para actualizar el documento (y, si afecta al troceo, también a `/feat:plan`). No sigas implementando contra un diseño que ya no es cierto — `/feat:review` y `/feat:validate` leen el `03-design.md` como verdad y harán juicios equivocados si miente.

Si las desviaciones son pequeñas (renombrados, ajustes locales), no pasa nada: anótalas y sigue.

## 4.2 Check textual de contratos (antes de cerrar)

Si en §2.0bis copiaste contratos a `05-implementation.md`, **antes de marcar el build como hecho** hay que confrontar el código contra cada contrato citado. **No es un test que correr** — es una comparación textual deliberada que tú haces como agente, no delegada al test runner.
...
- Si hay build multi-MR/PR, deja la MR/PR actual como `in_progress` en `meta.json.mrs`; pasará a `merged` cuando `/feat:ship` confirme el merge.
