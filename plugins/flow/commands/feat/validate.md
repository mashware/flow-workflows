...
- **Record *what* you validated**, in the same write: `validated_sha` = `git rev-parse HEAD`, work-level and in this MR/PR's `mrs[]` entry. Green on a tree is a statement about that tree; `/flow:feat:ship §1` reads it to tell "the suite passed here" from "the suite passed on something else". Only when the phase advances.
