...
### 1.5.1 Focused knowledge query, then the inventory
...
**Then the inventory.** Run §3's prior inventory **now**, before any approach is proposed, and keep
its result for `03-design.md`'s "## What already exists". An approach proposed without it reinvents
what the repository already has, and a lens whose only job was to go and look for it spends an
agent on what one search answers for every advisor at once. §3 is then already done.
...
### 1.5.3 Approach panel (parallel fan-out, one round)

**Launch the advisors as parallel subagents** — one per lens, a single round, each blind to the
others; then you synthesise. **There is no debate round.** Advisors that share one base model share
its blind spots: a round in which they read and attack each other tends to converge on the most
confident answer rather than surface a new one, and it costs as many agents as the first round. The
diversity is in what each lens is asked to *optimise*, and the adversarial pass is §6's challenger,
which attacks the design that was actually chosen.
...
**You are the chairman.** The advisors are subagents; the synthesis and the ranking are yours, never
a subagent's — you hold the work's context and you write `03-design.md`.

**Advisors (parallel, blind to each other).** One subagent per lens, up to `fanout_max`. M → the
first two lenses; L → add `operations`, and also on a sensitive surface (authentication or
authorization, payments, personal data, a public contract, a migration):

| Lens | Brief |
|---|---|
| `minimum` | The **smallest** approach that solves the declared use case, built on what the inventory found before anything new (strict MVP) |
...
> `.claude/work/<TICKET>/01-context.md` for context and the effective FLOW configuration (base plus
> active harness overlay) for project conventions. What already exists in the repository:
> `<the §1.5.1 inventory>` — propose a new piece only where nothing there fits. Do not
...
**Then you synthesise.** A reply that arrived is already in your context and its file adds
nothing — **open only the files of the lenses that did not answer**, and only then. Rank the
approaches best to worst *for this case* (project fit and simplicity, not generic merit), weighing
each one's own "why it could be a bad idea" against the others' — rank on substance, not on which
advisor wrote best. State explicitly where the advisors **agreed** and where they **disagreed** —
the disagreement is the useful part.

`agents.fanout_tool` set → run the round through that tool instead of plain parallel subagents; the
lenses, the briefs and the ceiling do not change.
...
`agents.budget_max`. Synthesise from those that answered and record `N/M`.
...
> the effective FLOW configuration and `.claude/work/<TICKET>/01-context.md`), building on what
> already exists: `<the §1.5.1 inventory>`. For each: a
...
**M/L already ran this in §1.5.1** — reuse that result and skip to §4. XS/S: **before** launching the design subagents, list what the feature needs that **already exists** in code or DB. Launch an `Agent` with `subagent_type: Explore`:
