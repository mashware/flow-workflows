...
- **The review has to be about the tree you are about to push.** `phases_done` says a review happened; it cannot say what it looked at, so on its own it passes a review of code that has since changed — the commits after it get pushed having been read by nobody. Compare `meta.json.reviewed_sha` and `validated_sha` (this MR/PR's `mrs[]` entry in a multi-MR/PR work) against `git rev-parse HEAD`:
  - **Equal** → continue, nothing to report.
  - **Absent** (a work started before the shas were recorded) → say so in one line and continue. Absence is not a mismatch, and refusing here would strand every work already in flight.
  - **Different** → read the delta (`git log --oneline <sha>..HEAD`, `git diff --stat <sha>..HEAD`) and judge it by **what it touches**, never by how big it is. Only test files (`tests/`, `test/`, `spec/`, `__tests__/`, `*_test.*`, `*Test.*`, `*.test.*`, `*.spec.*`) and this work's own folder under `.claude/work/` → append both shas and the file list to `06-review.md` and continue **without asking**: a test written by `validate` after the review is the normal order of this flow, and stopping on it would turn this gate into noise the user learns to wave through. **Anything else** → stop and ask, in **every** `autonomy.mode`, no exceptions, with a single `AskUserQuestion`: *re-review the delta* (`/flow:feat:review`, the recommended default and the cheap option — it reads `<sha>..HEAD`, not the whole branch) · *ship it as it stands*, which first appends to `06-review.md` the two shas, the files, and the fact that the user accepted them unreviewed. Never resolve this one silently: an unreviewed push is exactly what the review phase exists to prevent, and `guided`/`auto` are the modes where nobody is watching.
...
**Cross-repo contract handoff**: pointing at the sibling is not enough when it has to build against a shape decided here. Applies to each `related_repos` entry that is not `done` **and** has `contract_handoff: "pending"`, when `03-design.md` §"External contracts" is not "none". The reason this step exists: `03-design.md` lives in `.claude/work/`, which is **git-ignored** — the literal contracts, the most expensive artifact of the whole flow, do not survive this session and never reach the other repo. `scope` does survive, and it is one line of prose. So the cheap half of the information is the only half that crosses, and the sibling reinvents routes, payload keys and error codes that were already decided.

1. **Select what crosses.** Only the contracts *that sibling consumes*: the ones whose "Known consumer" points at it. If "Known consumer" was left blank, ask the user which contracts that repo consumes rather than publishing all of them. Copy them **verbatim** from `03-design.md` (the same copy-don't-paraphrase rule as `/flow:feat:build` §2.0bis — a paraphrased contract is a new contract). Acceptance criteria, the ADR-light and "Internal behavioral contracts" do **not** cross: they are this repo's *how*, and in the sibling's ticket they are noise that buries the part that matters.
2. **Where.** A comment on the tracker ticket, which is the anchor flow already assumes both sides share (`/flow:feat:start` §3.5 sends the sibling to the *same* ticket) and which the sibling's `start` reads: `/flow:feat:start` §2.1 and `/flow:bug:start` §1.1 read the comment thread, not only the description, so a block published here is picked up there. Requires `tracker.tool` not `none`/empty and `meta.json.ticket` a real tracker id. **Fallback** when there is no tracker (or the ticket is local-only): write the block to a **versioned** file in this repo (`docs/contracts/<TICKET>.md` or wherever `conventions` puts docs — never under `.claude/work/`) and name that path in the reminder, so the location is at least reachable from the sibling's checkout of this repo.
3. **Preview, always.** Publishing to the tracker is outward-facing and the whole team reads it, so show the exact comment text and ask for confirmation with `AskUserQuestion` (publish / edit / skip) **in every `autonomy.mode`, no exceptions** — the same rule as the mandatory MR/PR preview in §3. Never publish unreviewed prose into a shared tracker.
4. **Record it.** On publish, set that entry's `contract_handoff` to `published → <ticket comment url or file path>`. On skip, leave it `pending` — the reminder fires again next ship, which is the intended behaviour, not a nag. Failure to post → warn in one line, leave it `pending`, never block the ship.

This is still "notes and reminds": the contract goes to the tracker or to this repo, never into the sibling's working tree. Not a hard gate — a skipped handoff must never stop a merged MR/PR from closing out.
