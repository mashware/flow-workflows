...
- **Gather the material once, not once per reviewer.** `npx flow-workflows@<version> bundle --checks --harness claude`
  ...
  **§1.5 runs first and adds `--rev` and `--base` to that call**: the pack is then one frozen
  ...
  cannot reach it: read the diff from git and carry on, once, without retrying variants.
...
- **`<version>` is this plugin's own, in every `flow-workflows` call on this page.** Read it once
  from `version` in `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` and substitute it. Left
...
- **`--harness claude` is not decoration.** Without it the CLI reads `FLOW.md` alone and never
...
- **`agents.exec_cmd` set → the panel runs from the CLI.** Not a choice to weigh: run
  `npx flow-workflows@<version> review --harness claude`, which spawns that command once per role over the pack above — **one
...
Two questions, answered here and nowhere later: **which tree** this round reads, and **against which base**. Neither is implicit: the checkout changes while the round runs, and `git.default_base` is not always the right base.
...
**The review base.** `reviewed_sha` records the tree the last passing review read, and `ship` trusts it enough to stop a push. Reading only the delta since it keeps a second review from re-reading every line the first one cleared, and keeps §2.0 from resolving the tier on the accumulated total — a 30-line delta on a 900-line branch is a 30-line review. Resolve:
...
- **A first review has no `reviewed_sha` and reads the whole branch**. The delta path only exists after a review that passed.
...
**The CLI resolves it; you obey the output and record any deviation.** `npx flow-workflows@<version> tier --harness claude --rev <candidate_sha> --base <review base>` prints, from the config it already parses: changed lines over the §1.5 range, diff size, `meta.json.size`, **effective size** (the lower of the two), the built-in `code-review` effort with and without the sensitive-surface bump, whether the panel runs, the resolved `agents.budget_max`/`fanout_max`, whether the measurement landed **within 10% under** a threshold that changed the tier, and which `quality.sensitive_paths` globs the diff matched, with the path that matched each.

Every step of that is arithmetic. A tier this agent derives itself writes the same artifact whether it measured the diff or eyeballed it — on the one number in this command that carries an incentive on it.
...
**Dedup keeps every source, not the first one.** A finding carries its **origin** from here to §8 — `built-in`, a panel role, or the pass that raised it (`§3.5`, `§3.6`, `§5`, `§5.5`) — and a finding more than one source raised lists all of them. Counted once, credited to each: a finding only one pass saw and a finding three sources raised say opposite things about that pass, and §9 records the difference.
...
Counters a reviewer abandoning a large diff early. **Only if the effective size of §2.0 is M or L**, and only
...
**When it runs.** All four conditions:
...
Read the real diff (`git diff --stat <review base>...<candidate_sha>`, §1.5), not the recorded size.
...
<if §6 did not run, one line saying which condition did not hold — size, diff size, count, or budget>
...
- **Record *what* you reviewed, and separately what you changed after reading it**, in the same write, work-level and — in a multi-MR/PR work — in this MR/PR's `mrs[]` entry too:
  - `reviewed_sha` = **the candidate of §1.5**, the revision the reviewers actually read. Not `git rev-parse HEAD`, which by then holds this round's own fixes — code no reviewer saw, recorded as reviewed, and `ship` finding it equal to `HEAD` every time.
  - `fixed_sha` = `git rev-parse HEAD` after this round's fixes, or **empty** when the round applied none. Two fields because they are two facts.
...
- **Stage what the review taught** (`knowledge.stage` set; silence by default). Review is where a project teaches the most, and this is the only moment its findings are in context. Candidates are findings about the **domain or the codebase's real behaviour**, not about this diff: a library that does other than its name says, a test disguise that kept the suite green over a live defect, a module rule the reviewers had to cite from its own prose, a measurement. A blocker fixed here is not knowledge; *why it was written wrong* may be. Same evidence rule as `/flow:feat:design` §8 — one line of evidence per finding or it is not staged. One call per finding, one line to the user («Staged N finding(s) for `ship`»). Never `knowledge.save` here.
- Overwrite `00-summary.md` whole (≤15 lines, flow-core §5).
- **Record what the agents missed** (flow-core §6.7, `agents.learn` not `off`; asking nothing): for each finding that survived, the panel member or named `agents.<role>` whose category it falls in and who did not report it, and the delegated `build` agent whose code it sits in → one `meta.json.agent_lessons[]` row each, only when the lesson passes the §6.7 test. The origins §2.1 carried are what make this possible — a finding with no origin teaches nobody.
- **Configuration this phase earned the right to ask about** (flow-core §0, `manual` only, at most one key per work, as an extra option on this same stop — never a question of its own): a `Cost:` line that went over `agents.budget_max` → `quality.review_depth`. In `guided`/`auto` neither is asked: the default is taken and recorded in `meta.json.defaults_used[]`.
