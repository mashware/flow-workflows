...
- **Record *what* you reviewed**, in the same write: `reviewed_sha` = `git rev-parse HEAD`, work-level and — in a multi-MR/PR work — in this MR/PR's `mrs[]` entry too. `phases_done` only says a review *happened*; the sha is what says which tree it was about, and it is what `/flow:feat:ship §1` compares before pushing. Write it whenever the phase advances, never when it does not: a review that ended in blockers reviewed nothing that stands.
