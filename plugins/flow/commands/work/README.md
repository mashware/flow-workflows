...
- **Understand before starting**: design-affecting doubts left after ticket + knowledge search + code → one `AskUserQuestion` with all of them before closing `/flow:feat:start` or `/flow:feat:brainstorm`.
...
- **Knowledge sources (full cycle)**: the `knowledge` section names a tool per **role** — `search`, `stage`, `read_staging`, `save` — any MCP (domain-memory, codegraph…), CLI or skill; a role that is empty degrades (no lookup / finding stays in the artifact / artifacts are the staging / `KNOWLEDGE.md`); `knowledge.timeout_s` (2 s) or any failure → continue silently. `domain_memory.enabled: true` is the legacy alias for the four domain-memory tools. **`knowledge.search`**: `/flow:feat:start`, `/flow:bug:start` (ticket keywords), `/flow:feat:brainstorm` (concept), `/flow:feat:design` (module + integrations), `/flow:bug:diagnose` (component), `/flow:bug:investigate` (cause). **`knowledge.stage`**: closing `/flow:feat:design` and `/flow:bug:investigate`, offer to stage non-obvious domain decisions (silence by default; per-branch). **`knowledge.read_staging`** then **`knowledge.save`** at `/flow:feat:ship` and `/flow:bug:postmortem`: offer to consolidate the "why" only.
