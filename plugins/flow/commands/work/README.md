...
- **Commits are user opt-in**: during `/flow:feat:build` and `/flow:bug:fix`, the agent **does not run `git commit` on its own**. After each `TaskCreate` step, it edits the files and reports a summary (files, lines, validation suggestion). Wait for you to decide: commit now, wait to validate locally first (test the UI, run the flow, read the diff), or continue without committing. This breaks the previous pattern of automatic WIP commits per step. User control is preferred: if you validate and commit periodically, there are still cuttable units in `/flow:feat:build` §2.3; if you prefer not to commit until the end, accept that you lose that granularity. The system rule *"NEVER commit changes unless the user explicitly asks you to"* takes precedence — commits in `/flow:feat:ship` and `/flow:bug:ship` count as explicitly authorized because that is the stated purpose of the command.
