...
- **Machine — the CI pipeline.** `/flow:work:green` gets a red pipeline back to green: fetch failing jobs → triage → fix at the root → push. Objective signal (red → fix → green), no debate. It **never green-washes** (no blind reruns, no skipping a check). Green is often a precondition for review, so it usually runs first.
...
- `/flow:work:green [mr-iid-or-url]` — the **machine** half of the between-`ship`-and-`merge` window: the open MR/PR's CI pipeline is **red**. Fetches the failing jobs and logs via `git.cli`, triages each (lint/style, test failure, type/build, flaky/infra, quality-gate), fixes it **at the root** (delegating to the flow's sub-agents, reproducing locally with `quality.*` commands), and pushes — with hard gates on every push and rerun, and it **never green-washes** (no blind reruns, no disabling/skipping a check to force green — the analog of `respond` never resolving). Repeatable: one invocation per red pipeline, logged to `09-ci.md`. It does not advance `meta.json.phase`.
