...
**Challenger** — one subagent: the `agents.performance` agent from `FLOW.md` (or `agents.persistence`; if both are empty, `Agent general-purpose` with the role in the prompt). It gets the fact sheet, the schema and the code of the query, and **not** the design's rationale, the ticket's prose, or your reasons: exactly as `/flow:feat:review §5.5` blinds its idiom audit, because a plausible written justification is what makes a reviewer stop looking. On **L** work, or on a query on a hot path, run a **second** challenger with the checklist split between them (respect `agents.fanout_max`; empty → 4). Brief:
...
**Defender** — answer each attack with **evidence, not intent**: the plan, the index actually chosen, the rows read, the measured time. "It is bounded because the batch is small" is not a defence; the number that shows the batch is small is. Use `agents.persistence` if it is set and the query is idiomatic to a stack it knows; otherwise the judge defends inline.
