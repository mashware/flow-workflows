...
The `Cross-repo:` line is only printed if `meta.json.related_repos` has entries not `done`; show each as `<repo>: <scope>`, appending ` — contract not handed over` when that entry's `contract_handoff` is `pending`. It flags that a **sibling repo still has a pending part** of this task. flow only surfaces what's recorded — it never scans or touches the other repo.
...
- If **either** threshold is exceeded: add `⚠ exceeds estimate` and suggest that `/flow:feat:build` applies §2.3 (the size thermometer: cut / continue / reopen).
...
- Read `05-implementation.md` (features) or `04-fix.md` (bugs) and extract the listed files.
