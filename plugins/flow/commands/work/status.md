...
<TICKET> — <title> [feat|bug] [XS|S|M|L]  ⏵ <current phase>
  ...
  Cost:        $0.71 measured over 2 phases · 1 phase not reported        (line only if any)
  Next:        <suggested next command>
```

- "MR"/"PR" from `git.host` (`gitlab` → MR, every other host → PR); no host → "MR/PR".
...
- `Cost:` only when `meta.json.cost[]` has entries (flow-core §4): sum the `usd` figures that are there, count the phases behind them, and count separately the entries whose `source` is `not reported`. A work with no entries shows **no line at all** — absence is not zero, and a `$0.00` under a work nobody measured is the one output that would make the block worth ignoring. The figures are the harness's own client-side estimates, not a bill; `flow cost` prints the same data phase by phase.
