...
- **Round budget — check it before starting the round, not after.** Read how many rounds this MR/PR has already spent: `meta.json.mrs[].respond_rounds` for this MR/PR (work-level `respond_rounds` when the work has no `mrs`), or, when the field is absent, count the round entries already in `08-feedback.md`. The ceiling is `quality.respond_max_rounds` from `FLOW.md` (empty = `3`, `0` = no ceiling). If this round would exceed it, **do not start it.** Stop and hand the negotiation back — in **every** `autonomy.mode`, `auto` included — reporting three things: (a) which threads are still open and what each is actually about, (b) what was already tried in each spent round, read from `08-feedback.md` rather than from memory, and (c) **where the disagreement sits** — the one sentence the reviewer and this flow do not agree on. A fourth round that repeats the third is not persistence; it is the loop failing in the one place nobody is watching, and the user finding out by reading a long artifact is finding out too late. Raising the ceiling, taking the thread over, or granting one more round is the user's call — never a default you take yourself.
...
- **Count the round in the artifact, not in your head.** Increment `respond_rounds` for this MR/PR in `meta.json` (its `mrs[]` entry; work-level when there are no `mrs`) in the same write that appends this round to `08-feedback.md`. A session resumed tomorrow has no other way to know how many rounds this MR/PR has spent, and the §1 ceiling is worth exactly as much as this counter. In lightweight mode (no work folder) there is nothing to write and no ceiling to enforce: say so once and carry on.
- **A repeated answer spends the budget and buys nothing.** If a reply this round says what an earlier round already said — same position, same reasoning, no new evidence — stop **now** instead of at the ceiling, and escalate that thread by name: your position, the reviewer's counter, and the fact that neither has moved. Re-arguing a settled thread is the failure this budget exists for, and a round that only rephrases is its earliest symptom.
