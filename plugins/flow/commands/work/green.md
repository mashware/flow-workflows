...
- **No new FLOW.md keys.** This command reuses `git.*`, `tracker.*`, `quality.*` (`test`, `test_one`, `static_analysis`, `style_fix`, `frontend_test`, `review_skill`), `agents`, `models.code`, `autonomy.mode`, and `domain_memory.*`. Nothing to configure beyond what the flow already needs.
