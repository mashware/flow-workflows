...
```bash
npx flow-workflows install opencode           # or: gemini | codex | hermes
npx flow-workflows install opencode project   # project-scoped variant (where applicable)
npx flow-workflows check                      # installed vs newest release

./install.sh opencode                         # the same, from a clone, without Node
./install.sh opencode project
```

Two installers, one behaviour: [`../bin/cli.mjs`](../bin/cli.mjs) is what `npx` runs and
`install.sh` is what a clone runs. They are separate implementations, so `adapter-smoke.py`
installs every harness **both ways against throwaway `HOME`s and fails if the two trees differ** —
the only thing that keeps them from drifting apart. `package.json` is generated by
`adapter-build.py` from the plugin manifest, so the published version can never lag the plugin's.

Either installer sweeps its own previous `flow-*` files and copies the **commands** where the
harness reads them, then puts under `~/.claude/flow/` what the plugin would read from
`${CLAUDE_PLUGIN_ROOT}`: **`CORE.<tool>.md`** (the shared rules every command reads once per
session), the **`CHANGELOG.md`** `/flow-news` · `/flow:news` shows, the **`plugin.json`** it
takes the installed version from, this harness's **config fragment**, and **`FLOW.template.md`**.
The last two land there rather than being cited in place because `npx` deletes the package
directory as soon as the command ends — a path into it would be dead by the time you opened it.
Merging the fragment into your `opencode.json` / `settings.json` / `config.toml` / `config.yaml`
is yours to do: neither installer touches your configs.
