...
Put opencode-specific values in `FLOW.opencode.md`. opencode reads it after `FLOW.md`, key by key;
an explicitly empty overlay key masks the base, and an absent key inherits it. A repo with only
