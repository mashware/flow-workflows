...
1. **Read the staging accumulated during the branch**: call `mcp__domain-memory__read_staging`. There may be findings staged in `/flow-bug-investigate` if the root cause revealed something non-obvious. That is the primary material.
...
4. If there are 1+ relevant findings, ask the user if they want to save them. If yes, invoke the `/flow-save-knowledge` command with the right angle (the lesson, not the code). If no, do not insist.
...
- Suggest `/flow-bug-ship`. If the postmortem produced prevention actions, propose opening separate tickets (they are not handled in this flow).
