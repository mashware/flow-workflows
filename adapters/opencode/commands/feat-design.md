...
Si `domain_memory.enabled` es `true` en `FLOW.md`: antes de inventariar el código, llama a `mcp__domain-memory__search_knowledge` con consultas orientadas al **módulo afectado** e **integraciones** que va a tocar el diseño. Esto suele descubrir decisiones de dominio invisibles desde el código (restricciones legales, supuestos de la integración, motivos de un acoplamiento histórico).

Lanza 2-4 consultas en paralelo. Tiempo de espera máximo 2s; si falla, sigue. Resultados relevantes van al inicio del diseño bajo "Contexto de dominio adicional" (§4 template). Si `domain_memory.enabled` es `false` o vacío, salta sin avisar.

## 3. Inventario previo (reutilizar antes de crear)

**Antes** de lanzar los subagentes de diseño, identifica qué de lo que la feature necesita **ya existe** en el código o en la base de datos. Esto evita que los subagentes propongan piezas duplicadas. Lanza un subagente (modo exploración) con un encargo del tipo:

> Para la feature `<título>` (ver `.claude/work/<TICKET>/01-context.md`), busca en el repo qué piezas relacionadas ya existen y podrían reutilizarse: entidades de dominio, value objects, repositorios, servicios, eventos, columnas o tablas, comandos/consultas CQRS, y endpoints similares. No propongas diseño — solo lista lo encontrado con una línea cada uno y su ubicación. Si la feature menciona conceptos como `<concepto1>`, `<concepto2>`, busca específicamente esos.
...
Carga primero las convenciones relevantes según el proyecto (ver `FLOW.md` sección `conventions`).

Lanza en **paralelo** los subagentes que correspondan según la feature y el tipo de proyecto. Los subagentes se invocan con `@nombre` según estén declarados en `agents/` del proyecto (ver `FLOW.md` campos `agents.*`); si el campo está vacío, usa un subagente de propósito general con el rol indicado en el prompt:

- **Siempre**: subagente de arquitectura con el encargo de proponer: módulo donde vive, entidades/value objects nuevos o modificados, comandos/consultas CQRS (si aplica), eventos, repositorios.
- **Si toca DB**: subagente de persistencia con el encargo de proponer mappings, migraciones necesarias, índices, y gestor de entidades apropiado.
- **Si toca API/HTTP**: subagente de API con el encargo de definir endpoint, DTO, ruta, seguridad y formato de respuesta (solo planificación, no implementación).
- **Si toca rendimiento crítico o rutas de alto tráfico**: subagente de rendimiento para que prevea riesgos N+1 o de carga.
- **Si toca seguridad (autenticación, pagos, datos sensibles)**: subagente de seguridad con el encargo de listar amenazas y mitigaciones del diseño propuesto.
...
- **No añadir mecanismos defensivos "por si acaso".** Cada validación, guard, reintento, cerrojo, alternativa de respaldo o caché propuesto debe ir acompañado del escenario **real y presente** que lo exige (con evidencia: un hallazgo de `domain-memory`, un archivo, un patrón de tráfico conocido). Si el escenario es hipotético o el sistema actual ya lo impide, **no se propone**. Resolver lo que pide el ticket hoy, no problemas futuros (YAGNI).
...
<resultados del search_knowledge enfocado de §2, o "sin hallazgos">
...
## Contratos externos
<Si este cambio toca una superficie consumida desde fuera (otro repo, otro módulo, cliente desplegado, worker, migración referenciada por nombre, métrica/dashboard, evento de dominio, ruta HTTP), declara aquí **cada contrato como literal**, no en prosa. Cualquier contrato que quede en prosa es ambiguo y será fuente de fallos en construcción. Si no hay superficie externa, escribe "ninguno" y pasa de largo.>
...
<Una fila por cada validación, guard, reintento, cerrojo, alternativa de respaldo, caché, idempotencia, cola o flag que el diseño introduce. Si no puedes nombrar un escenario REAL y PRESENTE que lo justifique, la pieza sobra — quítala del diseño.>
...
Antes de cerrar, **desafía el diseño** lanzando un subagente de propósito general con este encargo (autocontenido):
...
>
> 1. **Encaje y necesidad (ángulo dominante — busca lo que sobra)**: revisa cada mecanismo defensivo del diseño (validación, guard, reintento, cerrojo, alternativa de respaldo, caché, idempotencia, cola, flag). Para cada uno pregunta:
...
>    - Sé concreto en el porqué: "X sobra porque en este proyecto Y siempre ocurre antes, ver `<archivo>`/`<hallazgo domain-memory>`".
...
> 4. **Operación en producción**: vuelta atrás, observabilidad, migraciones online, efectos cruzados con workers/cachés/colas. Solo lo que aplique de verdad a este cambio.
>
...
Si la feature toca **dominio sensible** (pagos, autenticación, datos personales, contadores de uso/seguimiento), lanza **en paralelo** un segundo subagente de propósito general con foco específico en el dominio:
...
No avances al cierre con severidades altas sin respuesta. Las medias y bajas son informativas. **Un challenger que devuelve "nada sobra ni falta, el diseño está ajustado" es un buen resultado, no un fallo** — no fuerces hallazgos.
...
No invoques `save_knowledge` aquí — el guardado final está en `/feat-ship` con `read_staging` previo. Si `domain_memory.enabled` es `false` o vacío, salta sin avisar.
