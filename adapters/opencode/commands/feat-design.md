...
- If `size` is `XS`, suggest jumping to `/feat-build` and exit unless the user insists.
...
>
> Read `01-context.md` for the business objective. Output: markdown table `| Angle | Finding | Type (unnecessary/missing) | Severity |` with severities `high`/`medium`/`low`. Under 500 words. Do not invent problems to fill space — if an angle has no findings, say "no findings". It is perfectly valid (and desirable) for the result to say "the design is tight, nothing unnecessary or missing".
...
- **Consequences**: moving from M to L activates the full flow. Moving from M to S removes `/feat-plan` from the path. Explicitly warn the user of the flow change.
...
Do not invoke `save_knowledge` here — the final save is in `/feat-ship` with a prior `read_staging`. If `domain_memory.enabled` is `false` or empty, skip silently.
...
- Next step based on size:
  - **XS / S**: suggest `/feat-build` (single MR/PR, no need to plan a split).
  - **M / L**: suggest `/feat-plan` to decide how to break the work into independently mergeable MRs/PRs before implementing.
