...
<if `meta.json.mrs` has >1 entry: "MR/PR 2/4 of the delivery plan — see #1 (link) and pending #3, #4". Include links to already merged ones.>
...
- **Cancel**: exit without creating anything. Do not touch `meta.json`. The user can return to `/feat-ship` later.
...
1. **Read the staging accumulated during this branch**: call `mcp__domain-memory__read_staging`. This shows what `/feat-design` (and possibly other phases) already staged. That is the primary material to consolidate.
...
4. If there are 1+ relevant findings, ask the user whether to consolidate them. If yes, invoke `/save-knowledge` (which already does `read_staging` internally and orchestrates the save; you provide the context of what to consolidate). If no, do not insist.
...
**A) MR/PR merged successfully** (normal case):
- If there are **no** `mrs` or only one: `phase = "done"`, add `ship` to `phases_done`, update `updated_at`.
- If it is a multi-delivery build: mark the current MR/PR as `merged` (with the final `url`) in `meta.json.mrs`. If there are `pending` entries remaining, leave `phase = "build"` (the cycle repeats for the next one). If all are `merged`/`closed`/`superseded`, `phase = "done"`.
...
- Ask the user: retry with another MR/PR (return to `/feat-build` with a different approach), or consider the feature unviable (`/work-abandon`)? Do not make the decision alone.
