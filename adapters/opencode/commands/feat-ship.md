...
## Qué NO incluye
<bullets sacados del "Esta MR/PR NO incluye..." del Brief. Importante para que el revisor sepa qué dejar fuera del alcance de la revisión.>
...
<si `meta.json.mrs` tiene >1 entrada: "MR/PR 2/4 del plan de entrega — ver #1 (link) y siguientes pendientes #3, #4". Incluir links a las anteriores ya fusionadas.>
...
Después pregunta al usuario (encabezado: "Crear <git.request_term>"):
...
```
- Si HEAD es la rama principal (master/main): para y avisa. No se envía desde la rama principal.
- Si el upstream es `<git.default_base>` (rama mal creada sin `--no-track`): **no envíes resolviendo upstream**. Corrige con `git branch --unset-upstream` y usa `git push -u origin HEAD`, que fija el upstream a la rama propia.
- En modo tren (`stacked_on` ≠ null): el MR/PR debe apuntar a esa rama padre, no a la base principal.

### 4.1 Crear MR/PR
Solo aquí — con el contenido aprobado por el usuario en §3 — haz commit, push y crea el MR/PR usando el CLI de `git.cli` de `FLOW.md`. Si hay un skill de commit+push+MR disponible en la herramienta, úsalo pasándole **título y descripción ya finales**; si no está disponible, hazlo manualmente en pasos separados. El push debe ser `git push -u origin HEAD` (rama propia), nunca a la rama base.
...
- **GitHub**: una conversación de revisión que requiera resolución antes de fusionar (política "require conversation resolution").
...
Es el freno: con la política del repo de "todos los hilos resueltos antes de fusión", el MR/PR no se puede fusionar ni desplegar hasta ejecutar el SQL y **resolver** el hilo. **Un solo hilo aunque haya varias sentencias.** Avisa al usuario de que queda abierto a propósito.
...
2. **Revisa los artefactos** `03-design.md`, `05-implementation.md` y `06-review.md` por si hay hallazgos del tipo "por qué" (decisiones de dominio, restricciones legales, integraciones, motivaciones de negocio) que **no se stagearon en su momento**. El "qué" (código, rutas) NO se guarda — eso está en el repo.
...
4. Si hay 1+ hallazgos relevantes, pregunta al usuario si quiere consolidarlos. Si dice sí, invoca el comando `/save-knowledge` (que ya hace `read_staging` internamente y orquesta el guardado; tú aportas el contexto de qué consolidar). Si dice no, no insistas.
...
**A) El MR/PR se fusionó correctamente** (caso normal):
- Si **no** hay `mrs` o solo había 1: `phase = "done"`, añade `ship` a `phases_done`, actualiza `updated_at`.
- Si es construcción multi-entrega: marca la MR/PR actual como `merged` (con `url` final) en `meta.json.mrs`. Si quedan entradas `pending`, deja `phase = "build"` (el ciclo se repite para la siguiente). Si todas están `merged`/`closed`/`superseded`, `phase = "done"`.

**B) El MR/PR se cerró sin fusión** (rechazada, descartada por revisores):
