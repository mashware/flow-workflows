...
2. La oferta de `save-knowledge` ya se hizo en `/bug-postmortem` — aquí no se vuelve a preguntar.
...
**Importante**: en este paso **no** se invoca aún commit ni push ni se crea nada. Solo se redacta el contenido para mostrárselo al usuario en §2.
...
<si hay `99-postmortem.md`: resumen ejecutivo de 3-5 bullets + enlace al artefacto en el repo o wiki. El resumen ejecutivo va aquí porque interesa a partes interesadas no técnicas; el detalle se lee aparte.>
...
Después pregunta al usuario (encabezado: "Crear {request_term}"):
...
No invoques commit ni push hasta confirmación explícita.

## 3. Commit, push y creación del MR/PR

### 3.0 Cerrojo anti-despliegue (antes de cualquier push)

Igual que `/feat-ship` §4.0: `git rev-parse --abbrev-ref HEAD` no debe ser la base principal (master/main), y `@{u}` no debe apuntar a `git.default_base`. Si el upstream apunta a la base, `git branch --unset-upstream` y `git push -u origin HEAD`. En modo tren el MR/PR apunta a la rama padre.

### 3.1 Crear MR/PR

Solo aquí — con el contenido aprobado en §2 — haz commit, push y crea el MR/PR usando el CLI de `git.cli` de `FLOW.md`. Si hay un skill de commit+push+MR disponible en la herramienta, úsalo pasándole **título y descripción ya finales**; si no está disponible, hazlo manualmente. El push debe ser `git push -u origin HEAD`, nunca a la base principal.
...
Con la política de "todos los hilos resueltos antes de fusión", el MR/PR no se puede fusionar ni desplegar hasta ejecutar el SQL y resolver el hilo. **Un solo hilo aunque haya varias sentencias.** Avisa al usuario de que queda abierto a propósito.
