...
- If the fix was developed in a worktree (`meta.json.worktree` not null), offer it once so the user can test this branch against the main environment: "run `/work-try <meta.branch>` (switches the main checkout and re-syncs per `git.worktree_resync`); `/work-try --back` to return." Suggest it, don't run it yourself.
