...
1. **Revisión integrada**: ejecuta una pasada completa sobre el diff local buscando fallos de correctitud + reutilización/simplificación/eficiencia, a esfuerzo alto. Si la herramienta dispone de un skill o comando de revisión de código incorporado, úsalo; si no, realiza la revisión tú mismo.
2. **Panel de proyecto**: lee `quality.review_skill` de `FLOW.md`.
   - Si `review_skill` tiene valor: invoca ese skill pasándole como contexto adicional el `03-design.md`. Alcance: `git diff <git.default_base>...HEAD`; si hay cambios en árbol de trabajo sin commitear, asegúrate de que entren.
   - Si `review_skill` está vacío pero `quality.reviewers` tiene entradas: lanza en paralelo cada subagente `@nombre` de esa lista como panel, con el mismo contexto y alcance.
   - Si ambos están vacíos: la pasada del punto 1 ya cubre esta revisión; no se lanza nada adicional.
...
- DB / consultas pesadas → usa el subagente de `agents.performance` de `FLOW.md` sobre los archivos cambiados; si está vacío, salta este refuerzo.
- Workers / colas de mensajes → usa el subagente de `agents.queues` de `FLOW.md` para verificar que no haya `flush()` en bucle y que los workers estén registrados con la convención del proyecto (ver `FLOW.md` sección `conventions`); si está vacío, salta este refuerzo.
- Frontend → si hay cambios en código de interfaz, usa el subagente de `agents.frontend` de `FLOW.md`; si además hay tests de frontend afectados, usa también `agents.frontend_test`; si alguno está vacío, salta ese refuerzo.
...
1. **Lista de trabajo**: `git diff --stat <git.default_base>...HEAD` → lista de ficheros/áreas cambiados.
...
3. **Crítico de completitud (1 subagente, sesgado)**: lánzalo pasándole **solo** dos cosas: la lista completa de ficheros del diff (paso 1) y, por cada revisor de §2-§3, una línea de qué cubrió. **No** le pases los hallazgos en detalle ni el diseño — su único trabajo es detectar huecos, no opinar sobre lo ya visto. Prompt:
...
6. **Sin truncado silencioso**: si tras 2 rondas el crítico sigue señalando áreas sin cubrir, **anótalas literalmente** en el output bajo "Áreas no cubiertas tras 2 rondas" con su motivo.
...
1. **Localiza todo mecanismo defensivo en el diff**: validación, guard, reintento, cerrojo, alternativa de respaldo, caché, idempotencia, circuit breaker, cola, flag, reintento.
...
3. **Verifica el escenario contra el código, no contra el papel**: ¿el flujo realmente puede llegar a ese estado? ¿hay un upstream que ya lo impide? Si `domain_memory.enabled` es `true`, consulta `mcp__domain-memory__search_knowledge` si el escenario depende de reglas de dominio.
...
## 5. Verificación doble ciega de contratos

Si `05-implementation.md` tiene sección "Contratos a respetar", lanza un subagente de propósito general con un prompt **deliberadamente sesgado**: solo recibe dos cosas, nada más, nada menos.
...
Por qué sesgado: si pasas el código completo o el diseño completo, el subagente racionaliza el desajuste leyendo justificaciones cercanas. Sesgándolo a "contrato literal vs lo que el diff emite", la comparación queda en lo textual.

Cualquier "no" en la tabla → bloqueante. Pasa al output como hallazgo de contrato roto con la propuesta concreta de ajuste.

Si `05-implementation.md` no tiene "Contratos a respetar" (construcción registró "N/A"), salta este paso.

## 6. Verificación adversarial de hallazgos (subagentes en paralelo, opcional M/L)

Los revisores tienden a **sobre-reportar**: un hallazgo "plausible" no siempre es real, y arreglar falsos positivos cuesta tiempo y a veces empeora el código. Si `meta.json.size` es **M o L** y la suma de bloqueantes + sugerencias de §2-§5 es **≥ 4**, ofrece al usuario filtrarlos ("¿Verificar los hallazgos con un panel de escépticos en paralelo? Descarta falsos positivos antes de que te los lleves a arreglar."). Si acepta, lanza en paralelo varios subagentes escépticos:

Para cada hallazgo `{id, archivo, descripcion, propuesta}`, lanza 3 subagentes independientes con el mismo prompt:
...
> Propuesta: {propuesta}
>
> Intenta REFUTARLO: lee el código real y di si el problema NO es real (refutado=true) o sí lo es (refutado=false). Ante la duda, refuta — la carga de la prueba es del hallazgo. Sé concreto sobre por qué.

Un hallazgo **sobrevive** si menos de 2 de los 3 escépticos lo refutan. Los **descartados** (≥2 escépticos los refutan) salen del listado de bloqueantes/sugerencias y se anotan en el output bajo "Descartados por verificación" con el motivo. No se ofrece en XS/S ni con menos de 4 hallazgos: el coste no compensa.
...
## Verificación doble ciega de contratos
