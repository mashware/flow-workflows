...
3. **Re-sync the environment** with `git.worktree_resync`. Because these commands can be invasive (schema migrations, container rebuilds), show the exact list and ask the user to confirm before running (**Run** / **Skip**). On confirm, run them **in order**, stopping and reporting if one fails. If `git.worktree_resync` is empty, skip this step silently.
