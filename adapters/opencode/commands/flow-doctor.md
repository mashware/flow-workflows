...
- **The `flow-workflows` CLI, which `review` reaches for before it composes a panel**:
  ...
  `~/.claude/flow/.claude-plugin/plugin.json` — the same pin `review` uses, so the number that
...
- **The plugin is one copy, not two.** The version in `~/.claude/flow/.claude-plugin/plugin.json`
...
- The `flow-core` skill file exists under the plugin root: `~/.claude/flow/CORE.opencode.md`.
