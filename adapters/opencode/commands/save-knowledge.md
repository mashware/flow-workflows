...
1. **Lee el staging** de la rama actual con `mcp__domain-memory__read_staging`. Si está vacío y tampoco tienes hallazgos nuevos en el contexto de la sesión actual, dile al usuario *"No hay nada que consolidar en esta rama."* y termina.
...
3. **Para cada hallazgo consolidado**:
   - Llama a `mcp__domain-memory__search_knowledge` con el topic y los `file_paths` del hallazgo.
   ...
   - Si no hay conflicto, llama a `mcp__domain-memory__save_knowledge` con la decisión.
