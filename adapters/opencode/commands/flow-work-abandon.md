...
- If the folder has a `panel.json` (the live panel described in the `work:README` guide), overwrite it with a terminal state before archiving — the title, one `block` line saying the work was abandoned and why, and no `Decision` line. The file travels with the folder into `_archive/`, so the user's panel stops finding the work at its old path either way; this is so the archived copy does not read as a work still in flight.
