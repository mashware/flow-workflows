...
Same two non-negotiable rules as `/feat-start` §5 (breaking them already caused an accidental deployment):
...
Summarize and suggest the next command based on size (`/bug-fix` for XS, `/bug-diagnose` for the rest). Then apply the `autonomy.mode` from the preamble: `manual` stops and recommends; `guided`/`auto` chain into that command automatically, subject to the hard gates.
