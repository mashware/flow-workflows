...
**Defender** — answer each attack with **evidence, not intent**: the plan, the index actually chosen, the rows read, the measured time. "It is bounded because the batch is small" is not a defence; the number that shows the batch is small is. Use the `agents.persistence` subagent if it is declared and the query is idiomatic to a stack it knows; otherwise the judge defends inline.
