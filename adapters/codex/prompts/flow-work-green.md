...
From `git`: `host`, `cli` (`glab`|`gh`; empty → from `host`), `request_term`, `default_base`. From `tracker`: `tool`/`prefix`. From `quality`: `test`, `test_one`, `static_analysis`, `style_fix`, `frontend_test` (local reproduce/verify in §5; empty → auto-discover), `review_skill` (§5). From `agents`: sub-agents to delegate to. If `domain_memory.enabled`, `search_knowledge` in §3 (skip silently if it fails or takes longer than 2 s). Follow any `notes` for this command (or `all`).
...
**2.3 Nothing to do?** Decide on the **combination**, never the pipeline alone. Green **and** mergeable → report both and stop. Green **but blockers remain** → never "green, you're good": report the green, list the blockers, continue (this is the case the command exists for). Still running → Codex has no in-session polling or auto-reschedule primitive, so say so plainly and let the user re-invoke this command once CI settles (cron + `codex exec "/flow-work-green"` for an unattended re-check); if non-pipeline blockers remain, work those meanwhile instead of idling. Never ran (no CI) → say so; if the verdict is clean too, stop.
...
- **Watch back to green**: the push re-runs the pipeline and this adapter cannot watch it — say so instead of implying it is being watched, and leave it to a later `/flow-work-green` re-check; do not claim fixed until CI reports green. Then **re-read the merge verdict** (§2.1): only that shows it is actually mergeable now.
