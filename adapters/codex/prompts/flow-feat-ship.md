...
- If there are **no** `mrs` or there was only 1: add `ship` to `phases_done` and update `updated_at`. `phase = "done"` **only if the user confirms it was actually merged** — ask once, because right after §4.1 the MR/PR is open and the normal answer is no. Not merged → leave `phase = "ship"`: `/flow-work-green` and `/flow-work-respond` operate on the open MR/PR and `/flow-work-clean` only sweeps what merged, so a work parked at `done` with its MR/PR open sends all three at the wrong target.
