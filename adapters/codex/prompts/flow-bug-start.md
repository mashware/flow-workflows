...
1. **Tracker** *(ticket mode only)*: read it using `tracker.view_cmd` from FLOW.md (substitute `{TICKET}` with `$ARGUMENTS`). **Read it whole, comment thread included — §1.1.** If `tool:none` or the key is missing, ask the user for the symptom, severity, and environment. **In ticket-less mode skip this — §1.5 synthesis is the source of symptom/severity/environment.**
