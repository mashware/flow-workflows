...
| S    | start → design (short) → build → review → validate → ship         | start → diagnose → fix → validate → review → ship  |
...
- `/flow-work-query` — the **query duel**: puts a data-access query on trial and settles it with an execution plan instead of an argument. Fact sheet (filter, order **with direction**, bound and whether it is per key or global, both sides of every join with their real types and collations, heavy columns, rows per key with its source, the indexes that actually exist — read from the schema, not the ORM mapping), a **blinded challenger** over the twelve classic failures, the main agent judging, **measurement** per `data.*` in FLOW.md (three runs per variant, plan next to time, plus `keys served`), and **one** recommendation with the number behind it. Invoked by the review phases, `respond`'s performance objections, `design`'s Access-paths table, `build`, `validate` and `investigate` — and directly, when the doubt arrives outside any phase. Judges, never patches; creating or seeding a database is a hard gate in every mode, never against production; with `data.*` empty it runs on the schema alone and says so.

## The query rule

**A query is judged on its plan, not on its prose**: correctness is visible in the code, cost is not. A query's cost lives in its **execution plan**, which depends on facts nowhere in the diff — which index exists, in what column order and **direction**, the type and collation of both sides of a join, how many rows a key really has. A panel of reading reviewers therefore approves a query that reads a hundred thousand rows to return fifteen, and does it faster when the design wrote down a plausible reason for it. So queries get their own adversarial pass (`/flow-work-query`): the facts stated, a challenger blinded to the design's rationale attacking them over a checklist of the classic failures, and the main agent judging — **no number, no win**, **no dogma in either direction** ("N small queries is an N+1" and "one batched query always wins" are both preferences until measured), and **the objector's variant measured next to yours**. It runs at **any size, XS included**. When a human raises a performance objection, the reply is a plan: a well-reasoned answer that has not looked at one is the most expensive thing this flow can produce, because it sounds verified and costs a review round to disprove. And an unmeasured plan is never reported as measured — the functional test database, with its handful of fixture rows, proves nothing about a plan.
