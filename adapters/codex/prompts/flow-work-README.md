...
- **`lines`** — one entry per line. A bare string is a plain line; an object takes `style` and `indent`. An empty string is a blank line. The reader wraps to its width and crops to its height, so a line that overflows is not truncated — it takes two ragged lines and loses its alignment.
...
**Contents, in this fixed order** so that several works read alike side by side: the work title; the MR/PR train (one line per `meta.json.mrs[]` entry — `#n`, short title, and its real state as the last column — with the URL indented under each entry still open, the not-yet-started ones collapsed into one `#a–#z` line, and the block omitted entirely when the work has no `mrs`); `Right now:`; `Next:`; `Waiting on you:` in `accent`, only when the flow is genuinely parked on the user; then `warn` blockers. Around 14 lines is the ceiling, and each line has to be short enough **not to wrap** (~55 characters): a wrapped line loses its column, and its continuation does not inherit `indent`.

**No headings over the train.** Grouping the entries under `Done` / `Now` / `Left` is the obvious layout and it is wrong here: in a train an MR/PR that has shipped is *open, waiting to merge*, so `Done  #1 …  MR open` contradicts itself in the one place the user is trusting at a glance. The state column already says it.
