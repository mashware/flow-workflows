...
2. **Revisa el postmortem** por si hay "por qué" adicionales (decisiones de negocio, restricciones legales, integraciones, supuestos del modelo que eran falsos) que no se stagearon. El "qué" (código, rutas) no se guarda.
3. **Combina staging + hallazgos nuevos**. Si queda vacío o solo cosas obvias, no insistas.
4. Si hay 1+ hallazgos relevantes, pregunta al usuario si quiere guardarlos. Si dice sí, invoca el flujo `/save-knowledge` con el ángulo correcto (la lección, no el código). Si no, no insistas.
