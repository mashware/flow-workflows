...
flow only **notes and reminds** — it never touches or scans the other repo. When you get to that side you start a normal work there with the same ticket. `/flow-feat-design` and `/flow-feat-plan` refine this list if the design reveals a repo the conversation missed. Record each entry's `contract_handoff` as `pending` when that repo will consume a surface decided here, or `none` when it won't — `/flow-feat-design` §7.5 confirms it once the contracts actually exist.
...
If the ticket points at another repo and there is **no** published contract, say so in one line — *"the `<repo>` side is referenced but no contract was published; the literal shapes will have to be confirmed with that side, not assumed"* — and record it under the same heading. Naming the gap is the point: an absent contract is invisible otherwise, and what fills the silence is invention that looks like knowledge. **Silent by default**: no cross-repo signal in the ticket → skip this section entirely.

## 4. Classify size

Based on the description and context, propose a size and ask the user for confirmation (single question): **Only in `manual`.** In **`guided`/`auto`, do not ask**: take your estimate, record it with its one-line reason, and continue — the size is a route, not a commitment, and `brainstorm`/`plan` reclassify it when the work turns out bigger or smaller than it looked.
...
**Below XS there is no work.** Before recording the size, ask whether this belongs in the flow at all: a change describable in one sentence, in one file, needing nobody's review, whose whole test story is that the existing suite passes or does not — a typo, a version bump, a log level. Then **say so and offer the alternative**: name the edit and the commit message, and let the user take it by hand (one question, every mode, `auto` included — entering the flow is itself the decision, and it is theirs). Keep the work when it needs the ticket moved, when someone else has to understand later why it was done, or when it touches a schema, a contract, or anything with a rollback story. A work folder opened for a typo is not rigour; it is what teaches the user to stop reaching for these commands.
...
## Contracts received
<only if §3.6 applied. Either the contract block copied verbatim from the other side (with its source), or the one-line note that another repo is referenced and no contract was published. Omit the section entirely when there's no cross-repo signal.>
