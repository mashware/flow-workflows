...
Lanza 2-3 queries en paralelo. Tiempo de espera máximo 2s; si falla, sigue sin contexto y no avises al usuario. Anota los hits relevantes en `02-brainstorm.md` bajo "Conocimiento de dominio adicional" (no repitas lo ya en `01-context.md`). Si `domain_memory.enabled` es `false` o vacío, salta sin avisar.

## 3. Trabajo

### 3.0 ¿Panel multiagente o agente único?

- Si `meta.json.size` es **M o L**: ofrece el **panel de enfoques en paralelo** ("¿Generar las opciones con un panel de subagentes en paralelo? Más costoso en tokens, menos sesgo de una sola línea de pensamiento."). Si acepta → §3.A. Si declina → §3.B.
...
Lanza **cuatro subagentes en paralelo** (uno por lente), sin que se vean entre ellos para garantizar diversidad real, y luego un quinto que sintetiza y ordena los resultados:
...
Cada agente de lente recibe: el ticket, la ruta a `.claude/work/<TICKET>/01-context.md` y la lente que le toca. Devuelve: nombre del enfoque, qué es (una frase), módulos/capas afectados, riesgo principal, por qué podría ser mala idea.

El agente sintetizador recibe los cuatro veredictos estructurados y ordena de mejor a peor para ESTE caso (encaje en el proyecto + simplicidad), con una recomendación inicial de 2-3 líneas de justificación.

Con el resultado, rellena §4 (cada enfoque → una "Opción", la síntesis → "Recomendación inicial"). Si un subagente falla, simplemente no lo incluyas.
...
- Si confirma, actualiza `meta.json.size` y anota el cambio en `meta.json.notes` (p.ej. `"size: M→S tras brainstorm — opción elegida no requiere migración"`).
