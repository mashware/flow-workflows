...
Apply the repo's `conventions` from `FLOW.md` — free text (layers, patterns, prohibitions), so read it and follow it as written; it is the whole instruction, not a pointer to something else to load.
