...
- **Non-trivial rounds** (beyond nitpicks, or introducing new architectural pieces) → run `/flow-feat-review`'s machinery **scoped to this round's diff**: **§2.0 depth ladder** (effort by round size + sensitive-surface bump, panel when selected), **§4 over-engineering/YAGNI** + **§5.5 idiom/primitive audit (blind to the design's rationale)** — the two that catch this loop's failure mode (extracting a class/mechanism to "answer a comment" that a reviewer then flags as the wrong primitive; §5.5 **always** runs when the round introduces new pieces, regardless of size), **§5 contract check** (if `05-implementation.md` lists contracts and the round touches shape construction), **§7 local gates** (`style_fix`/`static_analysis`/`test_one`). **Lightweight mode** (no `03-design.md`): skip §5, §4 judges YAGNI against the code itself; **§5.5 still runs**.
