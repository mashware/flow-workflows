...
5. **Publish to npm**: `npm publish` from the repo root, which is what `npx flow-workflows`
   serves to opencode, Gemini and Hermes users. `package.json` is generated by
   `adapter-build.py` from the plugin manifest, so its version is already right — never edit it
   by hand. Check the tarball first with `npm pack --dry-run`: it carries `bin/`, `adapters/`
   and the three plugin files the installer reads, and nothing else. Skipping this step is not
   fatal but leaves those three harnesses a release behind with no sign that they are.
